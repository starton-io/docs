...
export const createAutoSpacing = (theme: Theme) => {
    return (...args: ReadonlyArray<string | number>) => {
        return args.map((key) => (key && typeof key === 'string' ? `${theme.autoSpacingValues[key]}px` : '0px')).join(' ')
    }
}

export const createAutoSizing = (theme: Theme) => {
    return (...args: ReadonlyArray<string | number>) => {
        return args.map((key) => (key && typeof key === 'string' ? `${theme.autoSizingValues[key]}px` : '0px')).join(' ')
    }
}
