...
In the same way that we use human language code (the .sol file), the machine needs a version in its own language to work properly. This is exactly what the bytecode consists in: the human readable contract transformed into a machine readable format.
...
Scroll all the way down, to access the Bytecode and ABI.

Click on the ABI to copy on the clipboard.

The bytecode is not under the right format.
