# How to create a JWT-based authentication with NestJS

⌛ 1.5 hours

🟡 intermediary difficulty

[https://github.com/delicious-mango/jwt-auth-tuto](https://github.com/delicious-mango/jwt-auth-tuto)

<aside>
💡 In this tutorial, you will learn how to create a JSON Web Token (JWT) based authentication system with NestJS - a progressive Node.js framework. JWT is a standardized and widely used method for securely transmitting information between parties. It is commonly used for authentication and authorization purposes in web applications.
...
- `prisma` - an ORM to interact with the DB
...
To install these development packages cd into `jwt-auth-tuto`, run the following command:
...
- `@nestjs/jwt` - NestJS package for JWT-based authentication
- `bcrypt` - a package to hash passwords
- `@prisma/client` - the database client for the Prisma ORM that provides a type-safe API for querying your database.
- `class-validator`: allows you to easily define validation constraints for properties of your classes using decorators.
- `class-transformer`: allows you to transform plain JavaScript or TypeScript objects to instances of classes.
- `uuid`: a package to easily create ***U***niversally ***U***nique ***Id***entifiers
- `cookie-paser`: A middleware to parse `Cookie` header and populate `req.cookies` with an object keyed by the cookie names.
...
- The `id` field is a unique identifier for the user (it is mapped to _id in the DB)
- The `email` field is a unique field for the user's email address
- The `password` field is used to store the user's hashed password
...
<aside>
⚠️ Every time you make changes to your `prisma/shema.prisma` file, always remember to push the changes to your database to regenerate an updated Prisma Client.

</aside>

## Step 4: Wrap the Prisma Client
...
## Step 6: Import and configure the JwtModule
...
Let's create a user service to handle user-related tasks such as authentication, adding or fetching users from a database. Open the file named `src/user/user.service.ts` and add the following code:
...
The `signUp` method creates a new user in the database using the `create` method of the `prisma.user` object. It generates a JWT token using the `signAsync` method of the `JwtService` object, which is provided by the `@nestjs/jwt` package. It returns the `user` object and the `token`.

### Sign in

The `signIn` method checks if the user with the given email exists in the database using the `findUnique` method of the `prisma.user` object. If the user is found, it generates a JWT token for the user using the `signAsync` method of the `JwtService` object. It returns the `user` object and the `token`.

### Revoke token

The `revokeToken` method revokes a JWT token by adding it to the `revokedToken` collection in the database using the `create` method of the `prisma.revokedToken` object. It returns a `boolean`

### Get by id

The `getById` method retrieves a user by its Id from the database. If the user does not exist, it throws an exception. This method is used in the `getMe` method of the `UserController` to retrieve a user's data based on the JWT payload.
...
<aside>
⚠️ This don’t work on controllers in *library specific mode* (sign-up / sign-in routes)
...
In this tutorial, we have learned how to create a JWT-based authentication system with NestJS. We started by setting up a Prisma service and generating a Prisma Client, then created a user resource with a layered architecture. We then imported and configured the JwtModule, and created an authentication controller to handle authentication requests. After that we talked about Guards and how to use them to protect your routes. Finally, we created a user service to handle user-related tasks, and tested the authentication system.

---

[README.md](How%20to%20create%20a%20JWT-based%20authentication%20with%20Nest%202c63a314fd064cd3b30f9b2212390c72/README%20md%20f312dbab3457489fb0c7c29315544236.md)