---
title: How to notify your users when they receive an NFT?
---


# How to notify your users when they receive an NFT?
...
<aside>
⚠️ This tutorial is the continuation of the [How to create a JWT-based authentication with NestJS](https://www.notion.so/How-to-create-a-JWT-based-authentication-with-NestJS-2c63a314fd064cd3b30f9b2212390c72?pvs=21) tutorial. We recommend you to do it first before starting this one.
...
For this tutorial, we need to deploy an ERC1155 smart contract. You can do it via the Starton’s dashboard.

## Watcher  

To be able to receive notification from Starton about
...
<aside>
⚠️ `Red` fields are of *Non-scalar types* (non-native types, relations) such as the `items` field in the `User` collection are **not** represented in the underlying database
...
# Create a `Starton` module

To make our life easier we will create a `StartonModule`. This module will wrap API calls into utility methods.
...
For our app to be notified of a Transfer event, we need to set up a *Watcher* on Starton. A Watcher is a condition that is checked upon inspection of each block. When the watcher is triggered, it sends a POST request to a *webhook* containing data about the event. You can find how to create a Watcher [here](https://docs.starton.com/docs/Watcher/creating-a-watcher).

<aside>
💡 You can configure a *webhook* URL using ngrok. We already wrote a tutorial about it [here](https://docs.starton.com/docs/Tutorials/testing-webhooks-locally-ngrok).
