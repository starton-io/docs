...
With Starton, you can monitor blockchain activity. Whether it's to watch a wallet or the activity of a smart contracts, you can see create watchers, triggered by events.
An event is a set of  conditions that are checked when a block is inspected of the block. When the conditions of a watcher are met, the watcher triggers and sends a communication to the webhook given during the watcher's creation.
